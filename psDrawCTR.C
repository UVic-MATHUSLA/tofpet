...
int psDrawCTR(char const *filePrefix, Int_t channelA=-1, Int_t channelB=-1){
	...
	TH1F *hDelta = new TH1F("hDelta", "Coincidence time difference", 2*wMax/tBinWidth, -wMax, wMax);	// histogram for coincidence time difference
	
	const Int_t N_CHANNELS = 1024;
	TH2F *hC2 = new TH2F("hC2", "C2", N_CHANNELS, 0, N_CHANNELS, N_CHANNELS, 0, N_CHANNELS);	// histogram for 2D channel correlation
