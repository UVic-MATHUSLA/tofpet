...
PFP_KX7::~PFP_KX7() {
	setAcquistionOnOff(false);
	for(unsigned k = 0; k < N_BUFFER; k++) { delete[] bufferSet[k].data; }
	delete[] bufferSet;
	close(fd);
}
...
bool PFP_KX7::cardOK() { return true; }
...
int PFP_KX7::sendCommand(uint64_t *packetBuffer, int packetBufferSize) {
	...
	do {
		...
		if((now - startTime).total_milliseconds() > 10) { return -1; }
...
int PFP_KX7::setAcquistionOnOff(bool enable) {
	...
	if(enable) {
		...
		if(!bufferSetThreadValid) {
			...
			bufferSetRdPtr = 0;
			currentBuffer = NULL;
			for(unsigned k = 0; k < N_BUFFER; k++) { bufferSet[k].consumed = bufferSet[k].filled = 0; }
...
int PFP_KX7::setCoincidenceTrigger(CoincidenceTriggerConfig *config) {
	if(config == NULL || config->enable == 0) {
		...
		word32 = 0x00000000;
		for(int n = 0; n < 6; n++) { WriteAndCheck(base_addr0 + (CoincMasksReg + n) * 4, &word32, 1); }
	}
	else {
