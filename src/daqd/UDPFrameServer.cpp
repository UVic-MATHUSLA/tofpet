...
UDPFrameServer::UDPFrameServer(int udpSocket, const char *shmName, int shmfd, RawDataFrame *shmPtr, int debugLevel): FrameServer(shmName, shmfd, shmPtr, debugLevel), udpSocket(udpSocket) {}
	...
	while(!m->die) {
		...
		if(rxBuffer[0] == 0x5A) {
			...
			if(m->acquisitionMode != 0) {
				...
				//for(unsigned i = 0; i < nWords; i++) { dataBuffer[i] = be64toh(p1[i]); }
				...
				do {
					...
					pthread_mutex_lock(&m->lock);
					if(!isFull(m->dataFrameWritePointer, m->dataFrameReadPointer)) { dataFrame = &shmPtr[m->dataFrameWritePointer % MaxRawDataFrameQueueSize]; }
					pthread_mutex_unlock(&m->lock);
					...
					pthread_mutex_lock(&m->lock);
					if(dataFrame != devNull) { m->dataFrameWritePointer = (m->dataFrameWritePointer + 1) % (2 * MaxRawDataFrameQueueSize); }
		...
		else { printf("Worker: found an unknown frame (0x%02X) with %d bytes\n", unsigned(rxBuffer[0]), r); }
		pthread_mutex_lock(&m->lock);
...
int UDPFrameServer::sendCommand(int portID, int slaveID, char *buffer, int bufferSize, int commandLength) {
	...
	do {
		...
		if(reply == NULL) {
			...
			if((now - start).total_milliseconds() > CommandTimeout) break;
			else continue;
		}

		if(reply->size < 2) { fprintf(stderr, "WARNING: Reply only had %d bytes\n", reply->size); }
		else {
			...
			else { fprintf(stderr, "Mismatched SN: sent %6hx, got %6hx\n", sentSN, recvSN); }
		}
		delete reply;
		nLoops++;
	}
	...
	if(replyLength == 0) {
		printf(
			"Command reply timing: (t2 - t1) => %ld, (t3 - t2) => %ld, i = %d, status = %s\n",
...
uint64_t UDPFrameServer::getPortUp() { return 1; };

uint64_t UDPFrameServer::getDAQTemp() { return 0; };

uint64_t UDPFrameServer::getPortCounts(int port, int whichCount) { return 0; }
