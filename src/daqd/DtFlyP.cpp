...
// static void cacthINT1(int signal) {printf("1: SIG caught, %d\n", signal);}
// static void cacthINT2(int signal) {printf("2: SIG caught, %d\n", signal);}

const uint64_t idleWord = 0xFFFFFFFFFFFFFFFFULL;

bool DtFlyP::dmaBufferQueueIsEmpty() { return dmaBufferWrPtr == dmaBufferRdPtr; }

bool DtFlyP::dmaBufferQueueIsFull() { return (dmaBufferWrPtr != dmaBufferRdPtr) && ((dmaBufferWrPtr % NB) == (dmaBufferRdPtr % NB)); }
...
void *DtFlyP::runWorker(void *arg) {
	...
	while(!p->die) {
		...
		while(!p->die && p->dmaBufferQueueIsFull()) { pthread_cond_wait(&p->condCleanBuffer, &p->lock); }
		pthread_mutex_unlock(&p->lock);
		int status = ReceiveDMAbyBufIndex(DMA1, p->dmaBufferWrPtr % NB, 2);

		if(p->die) { status = ENOWORDS; }
		else if(status > 0) {
	...
	for(int i = 0; i < NB; i++) { ReleaseBuffer(i); }
...
int DtFlyP::getWords_(uint64_t *buffer, int count) {
	...
	if(dmaBufferQueueIsEmpty()) {
		...
		while(!die && dmaBufferQueueIsEmpty()) { pthread_cond_wait(&condDirtyBuffer, &lock); }
...
void DtFlyP::startWorker() {
	...
	// if(status != 0) { fprintf(stderr, "Error writing to register 0x002\n"); }
	if(hasWorker) return;
...
bool DtFlyP::cardOK() { return true; }
...
int DtFlyP::WriteAndCheck(int reg, uint32_t *data, int count) {
	...
	while(fail) {
		...
		// if(fail) { printf("DtFlyP::WriteAndCheck(%3d, %8p, %NBd) readback failed, retrying (%d)\n", reg, data, count, iter); }
		iter++;
	}
...
int DtFlyP::ReadAndCheck(int reg, uint32_t *data, int count) {
	...
	while(fail) {
		...
		fail = false;
		for(int i = 0; i < count; i++) fail |= (data[i] != readBackBuffer[i]);
		// if(fail) { printf("DtFlyP::ReadAndCheck(%3d, %8p, %NBd) readback failed, retrying (%d)\n", reg, data, count, iter); }
		iter++;
	}
	delete[] readBackBuffer;
	return status;
