...
Client::Client(int socket, FrameServer *frameServer): socket(socket), frameServer(frameServer) {}
...
int Client::handleRequest() {
	...
	if(cmdHeader.length > sizeof(socketBuffer)) return -1;
	...
	if(cmdHeader.type == commandAcqOnOff) doAcqOnOff();
	else if(cmdHeader.type == commandGetDataFrameSharedMemoryName) actionStatus = doGetDataFrameSharedMemoryName();
	else if(cmdHeader.type == commandGetDataFrameWriteReadPointer) actionStatus = doGetDataFrameWriteReadPointer();
	else if(cmdHeader.type == commandSetDataFrameReadPointer) actionStatus = doSetDataFrameReadPointer();
	else if(cmdHeader.type == commandToFrontEnd) actionStatus = doCommandToFrontEnd(nBytesNext);
	else if(cmdHeader.type == commandGetPortUp) actionStatus = doGetPortUp();
	else if(cmdHeader.type == commandGetPortCounts) actionStatus = doGetPortCounts();
	else if(cmdHeader.type == commandSetSorter) actionStatus = doSetSorter();
	else if(cmdHeader.type == commandSetTrigger) actionStatus = doSetTrigger();
	else if(cmdHeader.type == commandSetIdleTimeCalculation) actionStatus = doSetIdleTimeCalculation();
	else if(cmdHeader.type == commandSetGateEnable) actionStatus = doSetGateEnable();
	else if(cmdHeader.type == commandSetMinimumFrameID) actionStatus = doSetMinimumFrameID();
	else if(cmdHeader.type == commandGetDAQTemp) actionStatus = doGetDAQTemp();
...
int Client::doAcqOnOff() {
	...
	if(acqMode != 0) frameServer->startAcquisition(1);
	else frameServer->stopAcquisition();
