...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		if(c == -1) { break; }
		else if(c == 0 && optionIndex == 0) clientSocketName = (char *) optarg;
		else if(c == 0 && optionIndex == 1) debugLevel = boost::lexical_cast<int>((char *) optarg);
		else if(c == 0 && optionIndex == 2) {
			if(strcmp((char *) optarg, "GBE") == 0) { daqType = 0; }
			else if(strcmp((char *) optarg, "PFP_KX7") == 0) { daqType = 1; }
			else {
		...
		else if(c == 0 && optionIndex == 3) { daqCardList.push_back((char *) optarg); }
		else {
	...
	if(daqCardList.size() == 0) { daqCardList.push_back("/dev/psdaq0"); }

	if(daqCardPortBits == -1) {
		if(daqCardList.size() == 1) daqCardPortBits = 5;
		else daqCardPortBits = 2;
	}
	...
	if(clientSocket < 0) { goto cleanup_remove_client_socket; }

	FrameServer::allocateSharedMemory(shmName, shmfd, shmPtr);
	if((shmfd == -1) || (shmPtr == NULL)) { goto cleanup_shared_memory; }

	if(daqType == 0) { frameServer = UDPFrameServer::createFrameServer(shmName, shmfd, shmPtr, debugLevel); }
	else if(daqType == 1) {
		for(auto i = daqCardList.begin(); i != daqCardList.end(); i++) {
			AbstractDAQCard *card = PFP_KX7::openCard(i->c_str());
			if(card == NULL) goto cleanup_daq_cards;
	...
	if(frameServer == NULL) { goto cleanup_frame_server; }

	pollSocket(clientSocket, frameServer);
	retval = 0;

cleanup_frame_server:
	if(frameServer != NULL) { delete frameServer; }

cleanup_daq_cards:
	for(auto iter = daqCards.begin(); iter != daqCards.end(); iter++) { delete *iter; }
	...
	while(true) {
		...
		if(event.data.fd == clientSocket) {
			...
			if((event.events & EPOLLHUP) || (event.events & EPOLLERR)) {
				...
				delete clientList[event.data.fd];
