...
DAQFrameServer::~DAQFrameServer() { stopWorker(); }

void DAQFrameServer::startAcquisition(int mode) {
	stopAcquisition();

	for(auto card = cards.begin(); card != cards.end(); card++) { (*card)->setAcquistionOnOff(true); }
...
void DAQFrameServer::stopAcquisition() {
	...
	for(auto card = cards.begin(); card != cards.end(); card++) { (*card)->setAcquistionOnOff(false); }
}
...
static bool isFull(unsigned writePointer, unsigned readPointer) { return (writePointer != readPointer) && ((writePointer % MaxRawDataFrameQueueSize) == (readPointer % MaxRawDataFrameQueueSize)); }
...
void *DAQFrameServer::doWork() {
	...
	while(!die) {
		...
		if(!isFull(dataFrameWritePointer, dataFrameReadPointer)) { dst = &shmPtr[dataFrameWritePointer % MaxRawDataFrameQueueSize]; }
		...
		if(nCards == 1) {
			...
			for(; src_ptr < end_ptr; src_ptr++, dst_ptr++) *dst_ptr = *src_ptr | prefix;
		}
			...
			if((nEvents0 + nEvents1 + 2) > MaxRawDataFrameSize) { frameLost = true; }
...
int AbstractDAQCard::setSorter(unsigned mode) { return -1; }

int AbstractDAQCard::setCoincidenceTrigger(CoincidenceTriggerConfig *config) { return -1; }

int DAQFrameServer::setSorter(unsigned mode) { return -1; }

int DAQFrameServer::setCoincidenceTrigger(CoincidenceTriggerConfig *config) { return -1; }

int AbstractDAQCard::setGateEnable(unsigned mode) { return -1; }

int DAQFrameServer::setGateEnable(unsigned mode) { return -1; }
