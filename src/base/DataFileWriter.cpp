...
void DataFileWriter::openFile() {
	...
	if(fileType == FILE_ROOT) {
		...
		if(useAsyncWriting) { dataWriter = new DataWriter(fName2, true); }
		else {
		...
		if(useAsyncWriting) { dataWriter = new DataWriter(fName.c_str(), true); }
		else {
...
DataFileWriter::~DataFileWriter() {
	if(useAsyncWriting) { delete dataWriter; }
	else closeFile();
	if(fileSplitTime > 0) { renameFile(); }
};
...
void DataFileWriter::closeFile() {
	...
	else if(fileType == FILE_TEXT || fileType == FILE_TEXT_COMPACT) { fclose(dataFile); }
}
...
void DataFileWriter::writeRawEvents(EventBuffer<RawHit> *buffer, double t0) {
	...
	for(int i = 0; i < N; i++) {
		...
		else { fprintf(dataFile, "%lu\t%u\t%hu\t%hu\t%hu\t%hu\t%hu\n", hit.frameID, hit.channelID, hit.tacID, hit.tcoarse, hit.ecoarse, hit.tfine, hit.efine); }
	}
}
...
void DataFileWriter::writeSingleEvents(EventBuffer<Hit> *buffer, double t0) {
	...
	if(tMin > 0 && tMin > LLONG_MAX / 5000) {
		fprintf(
			stderr,
			"Error: User time reference exceeds available data precision for timestamps (tRef = %.0f s since UNIX epoch). Please choose a time reference within less than 100 days before data "
			"acquisition start\n",
	...
	for(int i = 0; i < N; i++) {
		...
		if(fileType == FILE_ROOT) {
			...
			Event eo = {((long long) (hit.time * Tps)) + tMin, hit.energy * Eunit, (int) hit.raw->channelID};
			if(useAsyncWriting) { dataWriter->appendData(static_cast<void *>(&eo), sizeof(eo)); }
			else { fwrite(&eo, sizeof(eo), 1, dataFile); }
		}
		else if(fileType == FILE_TEXT) { fprintf(dataFile, "%lld\t%f\t%d\n", ((long long) (hit.time * Tps)) + tMin, hit.energy * Eunit, (int) hit.raw->channelID); }
	}
}
...
void DataFileWriter::writeGroupEvents(EventBuffer<GammaPhoton> *buffer, double t0) {
	...
	for(int i = 0; i < N; i++) {
		...
		if(fileType == FILE_TEXT_COMPACT) { fprintf(dataFile, "%d\n", limit); }
		else if(fileType == FILE_BINARY_COMPACT) {
			GroupHeader header = {(uint8_t) limit};
			if(useAsyncWriting) { dataWriter->appendData(static_cast<void *>(&header), sizeof(header)); }
			else { fwrite(&header, sizeof(header), 1, dataFile); }
		}
		...
		for(int m = 0; m < limit; m++) {
			...
			if(fileType == FILE_ROOT) {
				...
				GroupEvent eo = {(uint8_t) p.nHits, (uint8_t) m, ((long long) (h.time * Tps)) + tMin, h.energy * Eunit, (int) h.raw->channelID};
				if(useAsyncWriting) { dataWriter->appendData(static_cast<void *>(&eo), sizeof(eo)); }
				else { fwrite(&eo, sizeof(eo), 1, dataFile); }
			}
			else if(fileType == FILE_BINARY_COMPACT) {
				Event eo = {((long long) (h.time * Tps)) + tMin, h.energy * Eunit, (int) h.raw->channelID};
				if(useAsyncWriting) { dataWriter->appendData(static_cast<void *>(&eo), sizeof(eo)); }
				else { fwrite(&eo, sizeof(eo), 1, dataFile); }
			}
			else if(fileType == FILE_TEXT) { fprintf(dataFile, "%d\t%d\t%lld\t%f\t%d\n", p.nHits, m, ((long long) (h.time * Tps)) + tMin, h.energy * Eunit, h.raw->channelID); }
			else if(fileType == FILE_TEXT_COMPACT) { fprintf(dataFile, "%lld\t%f\t%d\n", ((long long) (h.time * Tps)) + tMin, h.energy * Eunit, h.raw->channelID); }
		}
	}
}
