...
ProcessHit::ProcessHit(SystemConfig *systemConfig, EventStream *eventStream, EventSink<Hit> *sink): UnorderedEventHandler<RawHit, Hit>(sink), systemConfig(systemConfig), eventStream(eventStream) {
	resetCounters();
}
	...
	for(int i = 0; i < N; i++) {
		...
		if((in.channelID >> 12) == triggerID) {
			...
			if(useTDC) {
				...
				if(useTimeOffsetCal) out.time -= double(cc.t0) / clockPeriod;
