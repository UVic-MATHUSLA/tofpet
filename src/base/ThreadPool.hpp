...
namespace PETSYS {
	...
	template <class TEvent> class ThreadPool: public BaseThreadPool {
	  public:
		ThreadPool() {};
		virtual ~ThreadPool() {};

		void queueTask(EventBuffer<TEvent> *buffer, EventSink<TEvent> *sink) { BaseThreadPool::queueTask((void *) buffer, (void *) sink); };
	  private:
