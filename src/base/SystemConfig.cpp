...
void SystemConfig::touchChannelConfig(unsigned channelID) {
	...
	if(ptr == NULL) {
		...
		for(unsigned n = 0; n < 4096; n++) { ptr[n] = nullChannelConfig; }
...
SystemConfig::SystemConfig() {
	...
	for(int i = 0; i < MAX_TRIGGER_REGIONS * MAX_TRIGGER_REGIONS; i++) multihitTriggerMap[i] = false;
}
...
static void normalizeLine(char *line) {
	...
	s = boost::regex_replace(s, boost::regex("\\s*#.*"), "");   // Remove comments
	s = boost::regex_replace(s, boost::regex("^\\s+"), "");     // Remove leading white space
	s = boost::regex_replace(s, boost::regex("\\s+$"), "");     // Remove trailing whitespace
	s = boost::regex_replace(s, boost::regex("\\s+"), "\t");    // Normalize white space to tab
	strcpy(line, s.c_str());
}
