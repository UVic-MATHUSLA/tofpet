...
namespace PETSYS {
	template <class TEventInput, class TEventOutput> class OrderedEventHandler
		: public EventSink<TEventInput>,
		  public EventSource<TEventOutput> {
	  public:
		OrderedEventHandler(EventSink<TEventOutput> *sink): EventSource<TEventOutput>(sink) {
			pthread_mutex_init(&lock, NULL);
			expectedSeqN = 0;
			queue = std::map<size_t, pthread_cond_t *>();
		};

		~OrderedEventHandler() { pthread_mutex_destroy(&lock); };
		...
		virtual void pushEvents(EventBuffer<TEventInput> *buffer) {
			...
			expectedSeqN += 1;
			if(queue.count(expectedSeqN) > 0) { pthread_cond_signal(queue[expectedSeqN]); }
			pthread_mutex_unlock(&lock);

			this->sink->pushEvents(newBuffer);
		};

		virtual void finish() { this->sink->finish(); };

		virtual void report() { this->sink->report(); };

		virtual void resetCounters() { this->sink->resetCounters(); };
	  protected:
