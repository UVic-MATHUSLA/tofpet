...
namespace PETSYS {
	template <class TEventInput, class TEventOutput> class UnorderedEventHandler
		: public EventSink<TEventInput>,
		  public EventSource<TEventOutput> {
	  public:
		UnorderedEventHandler(EventSink<TEventOutput> *sink): EventSource<TEventOutput>(sink) {};

		~UnorderedEventHandler() {};

		virtual void pushT0(double t0) { this->sink->pushT0(t0); };
