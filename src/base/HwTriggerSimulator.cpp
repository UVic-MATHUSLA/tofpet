...
HwTriggerSimulator::HwTriggerSimulator(SystemConfig *systemConfig, EventSink<RawHit> *sink): UnorderedEventHandler<RawHit, RawHit>(sink), systemConfig(systemConfig) {
	nReceived = 0;
	nSent = 0;
}
...
EventBuffer<RawHit> *HwTriggerSimulator::handleEvents(EventBuffer<RawHit> *inBuffer) {
	...
	for(int i = 0; i < nTriggerRegions; i++) {
		for(int r: coincidenceRegionsAllowed[i]) {
			for(int tBin: triggerRequestTimeBins[r]) { triggerRequestCoincidenceTimeBins[i].push_back(tBin); }
		}
