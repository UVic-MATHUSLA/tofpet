...
int main(int argc, char *argv[]) {
	...
	while(true) {
		...
		if(c == -1) break;
			...
			switch(optionIndex) {
				...
				case 10:
					if(strcmp(optarg, "sync") == 0) tb = RawReader::SYNC;
						...
						fprintf(stderr, "ERROR: unkown timeref '%s'\n", optarg);
						exit(1);
					}
					break;
	...
	while(reader->getNextStep()) {
		...
		if(!simulateHwTrigger) {
			reader->processStep(true, new CoarseSorter(new ProcessHit(config, reader, new SimpleGrouper(config, new WriteGroupsHelper(dataFileWriter, new NullSink<GammaPhoton>())))));
		}
		else {
			reader->processStep(true, new HwTriggerSimulator(config, new ProcessHit(config, reader, new SimpleGrouper(config, new WriteGroupsHelper(dataFileWriter, new NullSink<GammaPhoton>())))));
		}
