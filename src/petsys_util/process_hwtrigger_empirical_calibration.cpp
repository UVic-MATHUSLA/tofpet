...
int main(int argc, char *argv[]) {
	...
	if(doSorting) { sortData(config, inputFilePrefix, outputFilePrefix); }

	calibrateAllModules(config, calibrationTable, outputFilePrefix);

	writeCalibrationTable(calibrationTable, outputFilePrefix);
	if(!keepTemporary) { deleteTemporaryFiles(outputFilePrefix); }
	return 0;
}
...
class WriteHelper: public OrderedEventHandler<GammaPhoton, GammaPhoton> {
  ...
  public:
	WriteHelper(EventWriter *eventWriter, EventSink<GammaPhoton> *sink): OrderedEventHandler<GammaPhoton, GammaPhoton>(sink), eventWriter(eventWriter) {};
...
void sortData(SystemConfig *config, char *inputFilePrefix, char *outputFilePrefix) {
	...
	while(reader->getNextStep()) {
		reader->processStep(true, new CoarseSorter(new ProcessHit(config, reader, new SimpleGrouper(config, new WriteHelper(eventWriter, new NullSink<GammaPhoton>())))));
...
void calibrateAsic(unsigned long gAsicID, int dataFile, CalibrationEntry *calibrationTable, char *summaryFilePrefix, SystemConfig *config) {
	...
	for(int n = 0; n < nTacs; n++) { hEnergy_vs_efine[n] = NULL; }
	...
	for(int n = 0; n < nChannels; n++) { hTotalEnergy[n] = NULL; }
	...
	while((r = read(dataFile, eventBuffer, sizeof(EnergyEmpiricalCalibrationEvent) * READ_BUFFER_SIZE)) > 0) {
		...
		for(int i = 0; i < nEvents; i++) {
			...
			if(event.gTacID - gTacStart >= nTacs) { continue; }
			if(event.gTacID - gTacStart < 0) { continue; }
	...
	while((r = read(dataFile, eventBuffer, sizeof(EnergyEmpiricalCalibrationEvent) * READ_BUFFER_SIZE)) > 0) {
		...
		for(int i = 0; i < nEvents; i++) {
			...
			if(event.time != -1E10) { hTime_vs_energy[event.gTacID - gTacStart]->Fill(event.energy, event.time); }
		}
	}
...
void writeCalibrationTable(CalibrationEntry *calibrationTable, const char *outputFilePrefix) {
	...
	for(unsigned long gid = 0; gid < MAX_N_TACS; gid++) {
		...
		fprintf(f, "%d\t%d\t%d\t%d\t%d\t%8.7e\t%8.7e\t%8.7e\t%8.7e\n", portID, slaveID, asicID, channelID, tacID, entry.p0, entry.p1, entry.p2, entry.k0);
...
void deleteTemporaryFiles(const char *outputFilePrefix) {
	...
	while(fscanf(tmpListFile, "%lu %[^\n]\n", &gAsicID, tmpDataFileName1) == 2) { unlink(tmpDataFileName1); }
	unlink(fName);
	fclose(tmpListFile);
}
