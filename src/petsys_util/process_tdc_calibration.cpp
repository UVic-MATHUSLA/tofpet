...
void calibrateAsic(
	...
	int linearityNbins,
	float linearityRangeMinimum,
	float linearityRangeMaximum,
	float frequency,
	CalibrationEntry *calibrationTable,
	float nominalM,
	char *summaryFilePrefix);

void calibrateAllAsics(
	int linearityNbins,
	float linearityRangeMinimum,
	float linearityRangeMaximum,
	CalibrationEntry *calibrationTable,
...
void calibrateAsic(
	...
	for(unsigned long gid = gidStart; gid < gidEnd; gid++) {
		...
		for(int i = 1; i <= nBinsX; i++) {
			if(pFine->GetBinEntries(i) > 0) nFilled++;
		}
		...
		float maxChi2 = 2E4;
		do {
			pf->SetParameter(0, tE);
			pf->SetParLimits(0, lowerT0, upperT0);
			pf->SetParameter(1, adcMin);
			pf->SetParLimits(1, adcMin - estimatedM * tEdgeTolerance, adcMin);
			pf->SetParameter(2, estimatedM);
			pf->SetParLimits(2, 0.98 * estimatedM, 1.02 * estimatedM), pFine->Fit("periodicF1", "Q", "", xMin, xMax);
		...
		do {
			...
			pf2->SetParameter(0, tE);
			pf2->SetParLimits(0, lowerT0, upperT0);
			pf2->SetParameter(1, a0);   // pf2->SetParLimits(1, 0.1, 200.0);
			pf2->SetParameter(2, a1);   // pf2->SetParLimits(2, 0.1, 300.0);
			pf2->SetParameter(3, a2);
			pf2->SetParLimits(3, -20, -0.01);   // Very small values of a2 cause rouding errors
			...
			if(currChi2 < minChi2) minChi2 = currChi2;
	...
	for(int iter = 0; iter <= nIterations; iter++) {
		...
		while((r = read(linearityDataFile, calDataBuffer, sizeof(CalibrationData) * READ_BUFFER_SIZE)) > 0) {
			...
			for(int i = 0; i < nEvents; i++) {
				...
				for(int j = 0; j < calData.freq; j++) {
					...
					if(fabs(tError) < ControlHistogramRange) { hControlE->Fill(tError); }
				}
			}
		}
		...
		for(unsigned long gid = gidStart; gid < gidEnd; gid++) {
			...
			if(pControlT->GetEntries() > 100) { offset = pControlT->GetMean(2); }
			...
			if(hControlE->GetEntries() > 1000) {
				...
				if(f != NULL) { offset = f->GetParameter(1); }
			}
...
void writeCalibrationTable(CalibrationEntry *calibrationTable, const char *outputFilePrefix) {
	...
	for(unsigned long gid = 0; gid < MAX_N_TAC; gid++) {
		...
		fprintf(f, "%d\t%d\t%d\t%d\t%d\t%c\t%8.7e\t%8.7e\t%8.7e\t%8.7e\n", portID, slaveID, chipID, channelID, tacID, bStr, entry.t0, entry.a0, entry.a1, entry.a2);
	}
	fclose(f);
}
...
void printCalibrationSummary(CalibrationEntry *calibrationTable, const char *outputFilePrefix) {
	...
	for(int gChipID = 0; gChipID < MAX_N_ASIC; gChipID++) {
		...
		printf(
			"│ %-11s│%6d    │%6.1f  │%6.1f  │%6.1f  │%6.1f  │%6.1f  │%6.1f  │\n",
			cid,
