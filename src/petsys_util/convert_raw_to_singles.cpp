...
int main(int argc, char *argv[]) {
	...
	while(reader->getNextStep()) {
		...
		if(!simulateHwTrigger) { reader->processStep(true, new CoarseSorter(new ProcessHit(config, reader, new WriteSinglesHelper(dataFileWriter, new NullSink<Hit>())))); }
		else { reader->processStep(true, new HwTriggerSimulator(config, new ProcessHit(config, reader, new WriteSinglesHelper(dataFileWriter, new NullSink<Hit>())))); }
