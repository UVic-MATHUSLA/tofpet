...
namespace PETSYS { namespace OnlineMonitor {
	Histogram1D::Histogram1D(int nBins, double lowerX, double upperX, string n, Monitor *m, char *p): Object(n, m, p) {
		this->nBins = nBins;
		this->lowerX = lowerX;
		this->upperX = upperX;
	...
	void Histogram1D::reset() { memset(ptr, 0, sizeof(double) * (nBins + 1)); }
	...
	int Histogram1D::getBin(double x) { return floor(x * binWidthReciprocal); }
