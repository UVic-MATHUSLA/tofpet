...
class Filler: public OrderedEventHandler<Coincidence, Coincidence> {
  ...
  public:
	Filler(char *tocFileName, Monitor *m, EventSink<Coincidence> *sink):
		OrderedEventHandler<Coincidence, Coincidence>(sink), monitor(m), acquisition_t0(NULL), acquisition_elapsed(NULL), coincidence_counter(NULL), channel_counter(), channel_energy() {
...
int main(int argc, char *argv[]) {
	...
	auto pipeline = new Decoder(
		totMode,
		new CoarseSorter(new ProcessHit(config, eventStream, new SimpleGrouper(config, new CoincidenceGrouper(config, new Filler(tocFileName, monitor, new NullSink<Coincidence>()))))));
