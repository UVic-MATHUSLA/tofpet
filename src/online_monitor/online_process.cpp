...
class Decoder: public UnorderedEventHandler<UndecodedHit, RawHit> {
  public:
	...
	void report() { UnorderedEventHandler<UndecodedHit, RawHit>::report(); };

	void resetCounters() { UnorderedEventHandler<UndecodedHit, RawHit>::resetCounters(); };
  protected:
...
Decoder *createProcessingPipeline(EVENT_TYPE eventType, OnlineEventStream *eventStream, SystemConfig *config, DataFileWriter *dataFileWriter) {
	Decoder *pipeline;
	if(eventType == RAW) { pipeline = new Decoder(eventStream, new WriteRawHelper(dataFileWriter, new NullSink<RawHit>())); }
	else if(eventType == SINGLE) { pipeline = new Decoder(eventStream, new CoarseSorter(new ProcessHit(config, eventStream, new WriteSinglesHelper(dataFileWriter, new NullSink<Hit>())))); }
	else if(eventType == GROUP) {
		pipeline = new Decoder(eventStream, new CoarseSorter(new ProcessHit(config, eventStream, new SimpleGrouper(config, new WriteGroupsHelper(dataFileWriter, new NullSink<GammaPhoton>())))));
	}
	else if(eventType == COINCIDENCE) {
		pipeline = new Decoder(
			eventStream,
			new CoarseSorter(new ProcessHit(config, eventStream, new SimpleGrouper(config, new CoincidenceGrouper(config, new WriteCoincidencesHelper(dataFileWriter, new NullSink<Coincidence>()))))));
	}
	return pipeline;
}
...
int main(int argc, char *argv[]) {
	...
	if(strcmp(eType, "raw") == 0) { eventType = RAW; }
	else if(strcmp(eType, "singles") == 0) { eventType = SINGLE; }
	else if(strcmp(eType, "groups") == 0) { eventType = GROUP; }
	...
	else if(strcmp(fType, "text") == 0) { fileType = FILE_TEXT; }
	else if(strcmp(fType, "textCompact") == 0) { fileType = FILE_TEXT_COMPACT; }
	else if(strcmp(fType, "root") == 0) { fileType = FILE_ROOT; }

	timeref_t tb;
	if(strcmp(tref, "sync") == 0) { tb = SYNC; }
	else if(strcmp(tref, "wall") == 0) { tb = WALL; }
	else if(strcmp(tref, "step") == 0) { tb = STEP; }
	else if(strcmp(tref, "manual") == 0) { tb = MANUAL; }
	...
	if(totMode) { mask ^= (SystemConfig::LOAD_QDC_CALIBRATION | SystemConfig::LOAD_ENERGY_CALIBRATION); }
	...
	if(strcmp(mode, "mixed") == 0) {
		...
		for(unsigned long gChannelID = 0; gChannelID < MAX_NUMBER_CHANNELS; gChannelID++) eventStream->setMode(gChannelID, strcmp(mode, "qdc") == 0);
	}
	...
	while(fread(&blockHeader, sizeof(blockHeader), 1, stdin) == 1) {
		...
		while(rdPointer != wrPointer) {
			...
			if(blockHeader.blockType == 1 && lastFrameType == FRAME_TYPE_ZERO_DATA && frameType == lastFrameType) { continue; }

			// Do not write sequences of all lost frames, unless we're closing a step
			if(blockHeader.blockType == 1 && lastFrameType == FRAME_TYPE_ALL_LOST && frameType == lastFrameType) { continue; }
		...
		if(blockHeader.blockType == 2) {
			if(verbose == true) {
				fprintf(
					stderr,
					"onlineProcessing:: Step had %lld frames with %lld events; %f events/frame avg, %lld event/frame max\n",
					stepAllFrames,
					stepEvents,
				...
				fprintf(
					stderr,
					"onlineProcessing:: some events were lost for %lld (%5.1f%%) frames; all events were lost for %lld (%5.1f%%) frames\n",
					stepLostFramesN,
