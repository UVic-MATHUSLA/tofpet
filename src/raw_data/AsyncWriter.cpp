...
DataWriter::DataWriter(const std::string &filename, bool acqStdMode) {
	if(filename == "/dev/null") {
		fd = -1;   // Intentionally set fd to -1 when writing to /dev/null
		return;
	}
...
DataWriter::~DataWriter() {
	...
	auto &currentBuffer = buffers[currentBufferIndex];                        // Use a normal write to write any data in the last buffer
	auto write_size = (currentBuffer.used / IO_BLOCK_SIZE) * IO_BLOCK_SIZE;   // but we still need to pad it to IO_BLOCK_SIZE
	...
	else if(r != write_size) { fprintf(stderr, "ERROR at '%s':%d pwrite (..., %ld, ...) returned %ld\n", __FILE__, __LINE__, write_size, r); }
	r = ftruncate(fd, global_offset + currentBuffer.used);   // Finally truncate the file to the correct size
	if(r != 0) {
...
void DataWriter::appendData(void *buf, size_t count) {
	...
	while(written < count) {
		...
		if(currentBuffer.used == BUFFER_SIZE) {
			submittCurrentBuffer();   // This will also move currentBufferIndex to the next buffer
		}
	}
}
...
void DataWriter::submittCurrentBuffer() {
	...
	if(currentBufferIndex == N_BUFFERS) {
		completeAllBuffers();   // All buffers have been submitted, complete them before proceeding
	}
}
